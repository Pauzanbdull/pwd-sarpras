  ...
  <div class="container mx-auto px-4 py-8 max-w-7xl relative z-10">
    <header class="flex flex-col md:flex-row justify-between items-start md:items-center mb-12 gap-6">
      <div>
        ...
        <p class="text-[#a3e0e0] mt-2">Kelola data pengembalian barang dengan mudah</p>
      </div>
      ...
      <table class="min-w-full text-white text-sm">
        <thead>
          <tr class="bg-[#005f73] text-left text-white uppercase text-xs">
            ...
            <th class="px-6 py-4">Nama Pengembali</th>
        ...
        <tbody>
          @forelse ($pengembalians as $index => $item)
            <tr class="border-t border-[#027373]/40 table-row">
              ...
              <td class="px-6 py-4">{{ $item->nama_pengembali }}</td>
              <td class="px-6 py-4">{{ $item->barang->nama ?? '-' }}</td>
              <td class="px-6 py-4">{{ \Carbon\Carbon::parse($item->tanggal_pengembalian)->format('d M Y') }}</td>
              <td class="px-6 py-4">{{ $item->kondisi_barang }}</td>
              ...
              <td class="px-6 py-4">
                <form action="{{ route('pengembalian.destroy', $item->id) }}" method="POST" onsubmit="return confirm('Yakin ingin menghapus data ini?')">
                  @csrf
                  @method('DELETE')
                  <button type="submit" class="inline-flex items-center gap-1 px-3 py-1.5 bg-red-500 hover:bg-red-600 rounded-md text-sm transition">
          ...
          @empty
            <tr>
              <td colspan="7" class="px-6 py-4 text-center text-white/70 italic">Tidak ada data pengembalian.</td>
            </tr>
          @endforelse
        </tbody>
      </table>
